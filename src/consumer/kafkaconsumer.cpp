...
int main (int argc, char **argv) {
    ...
    if(argc != 4){
		fprintf(stderr, "%% Usage: %s <broker> <topic> <partition>\n", argv[0]);
        return 1;
	}

    //设置broker
    broker.assign(argv[1]);
    //设置topic
    topic.assign(argv[2]);
    //设置partition
    partitionid = atoi(argv[3]);
    ...
    RdKafka::TopicPartition *partition = RdKafka::TopicPartition::create(topic, partitionid);
